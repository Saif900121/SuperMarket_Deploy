...
if page == 'Dataset Overview':
    ...
    * Branch performance comparisons
    ...
    f"""<h1 style="color:#f39c12; text-align:center;"> Top {top_n} Popular Products </h1>""",
     unsafe_allow_html=True)
    st.plotly_chart(px.bar(product_count, x = 'product_line', y = 'count'))

elif page == 'Bi-Variate Analysis':
    st.header('Does customer type (Member vs. Normal) affect total sales or gross income?')
    sales_per_type = df.groupby('customer_type')['sales'].sum().reset_index()
    st.plotly_chart(px.bar(data_frame= sales_per_type, x = 'customer_type', y= 'sales', text_auto= True,
       labels= {'sales': 'Total sales', }))
    income_per_type = df.groupby('customer_type')['gross_income'].sum().reset_index()
    st.plotly_chart(px.bar(data_frame= income_per_type, x = 'customer_type', y= 'gross_income', text_auto= True,
       labels= {'gross_income': 'Total gross income', }))
    ...
    total_income_per_product = df.groupby('product_line')['gross_income'].sum().sort_values(ascending=False).reset_index()
    st.plotly_chart(px.bar(data_frame= total_income_per_product, x= 'product_line', y= 'gross_income', text_auto= True))
    ...
    income_per_month = df.groupby(['month_name', 'payment', 'product_line'])['gross_income'].sum().reset_index()
    st.plotly_chart(px.bar(data_frame= income_per_month, x= 'month_name', y= 'gross_income', color= 'product_line',facet_col= 'payment', 
       barmode= 'group'))

    st.header('What combination of city, payment method, and product line leads to the total gross income?')
    income_per_line = df.groupby(['city', 'payment', 'product_line'])['gross_income'].sum().reset_index()
    st.plotly_chart(px.bar(data_frame= income_per_line, x= 'product_line', y= 'gross_income', color= 'payment',facet_col= 'city', 
       barmode= 'group'))
